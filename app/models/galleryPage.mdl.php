...
class galleryPage extends _site {
	public $GalleryImages = array();
	public $GalleryImageAddLink;

	public static function SetWordlets() {
		self::$view->Wordlets->AddWordlets('gallery');
	}

	public static function SetGalleryImages() {
		if ( self::$view->EditMode ) {
			$images = _site::Lade()->GetList('gimage', '', 'ladedgm_gimage.created', 'DESC');
		} else {
			$images = _site::Lade()->GetList('gimage', 'ladedgm_gimage.enabled=1 AND ladedgm_gimage.created<NOW()', 'ladedgm_gimage.created', 'DESC');
		}
