		...
		<div class="cms">
			<p>
				<? if ( !$page->Wordlets->GetWordlet('store_enabled') ): ?>
					<span style="color: white; font-weight: bold; background: red; display: inline-block; padding: 2px 6px">Store is disabled.</span> <?= $page->Wordlets->GetTag('store_enabled', 'Click here to enable', true) ?>.
				<? else: ?>
					<span style="color: white; font-weight: bold; background: green; display: inline-block; padding: 2px 6px">Store is enabled.</span> <?= $page->Wordlets->GetTag('store_enabled', 'Click here to disable', true) ?>.
				<? endif ?>
			</p>
			<p>
